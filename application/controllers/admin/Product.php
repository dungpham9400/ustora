...
class Product extends MY_Controller {
	...
	public function index()
	{
		...
		if ($this->input->post())
		{
			...
			if ($flag > 0) {
				$this->session->set_flashdata('message_success', 'Delete'.$flag.'product successfully');
			}else{
	...
	public function add()
	{
		...
		if ($this->input->post()) {
			...
			$this->form_validation->set_rules('catalog_id','Product','required');
			...
			if ($this->form_validation->run()) {
				...
				if ($this->product_model->create($data)) {
					$this->session->set_flashdata('message_success', 'Add product successfully');
				}else{
					$this->session->set_flashdata('message_fail', 'Fail to delete');
				}
				redirect(admin_url('product'));
	...
	public function edit()
	{
		...
		if (empty($product)) {
			$this->session->set_flashdata('message_fail', 'Product does not exist');
			redirect(admin_url('product'));
		}
		$this->data['product'] = $product; 
		if ($this->input->post()) {
			...
			if ($this->form_validation->run()) {
				...
				if ($this->product_model->update($id,$data)) {
					$this->session->set_flashdata('message_success', 'Edit product successfully');
				}else{
					$this->session->set_flashdata('message_fail', 'Edit to change');
				}
				redirect(admin_url('product'));
			}
		}
